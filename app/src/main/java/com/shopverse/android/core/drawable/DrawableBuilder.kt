package com.shopverse.android.core.drawable
...
import top.defaults.drawabletoolbox.FlipDrawableBuilder
import top.defaults.drawabletoolbox.RippleDrawableBuilder
import top.defaults.drawabletoolbox.RotateDrawableBuilder
import top.defaults.drawabletoolbox.ScaleDrawableBuilder
import top.defaults.drawabletoolbox.setColors
import top.defaults.drawabletoolbox.setGradientRadius
import top.defaults.drawabletoolbox.setGradientRadiusType
import top.defaults.drawabletoolbox.setInnerRadius
import top.defaults.drawabletoolbox.setInnerRadiusRatio
import top.defaults.drawabletoolbox.setOrientation
import top.defaults.drawabletoolbox.setStrokeColor
import top.defaults.drawabletoolbox.setThickness
import top.defaults.drawabletoolbox.setThicknessRatio
import top.defaults.drawabletoolbox.setUseLevelForShape
import java.util.TreeMap
import java.util.concurrent.atomic.AtomicInteger

class DrawableBuilder(private val context: Context) {

    fun startColor(color: AppColor) = startColor(color.value(context))
    fun centerColor(color: AppColor) = centerColor(color.value(context))
    fun endColor(color: AppColor) = endColor(color.value(context))
    fun gradientColors(startColor: AppColor, endColor: AppColor, centerColor: AppColor?) =
        gradientColors(
            startColor.value(context),
            endColor.value(context),
            centerColor?.value(context)
        )

    fun solidColorPressed(color: AppColor?) = solidColorPressed(color?.value(context))
    fun solidColorPressedWhenRippleUnsupported(color: AppColor?) =
        solidColorPressedWhenRippleUnsupported(color?.value(context))

    fun solidColorDisabled(color: AppColor?) = solidColorDisabled(color?.value(context))
    fun solidColorSelected(color: AppColor?) = solidColorSelected(color?.value(context))
    fun solidColor(solidColor: AppColor) = solidColor(solidColor.value(context))
    fun strokeColor(strokeColor: AppColor) = strokeColor(strokeColor.value(context))
    fun strokeColorPressed(color: AppColor?) = strokeColorPressed(color?.value(context))
    fun strokeColorDisabled(color: AppColor?) = strokeColorDisabled(color?.value(context))
    fun strokeColorSelected(color: AppColor?) = strokeColorSelected(color?.value(context))
    fun rippleColor(color: AppColor) = rippleColor(color.value(context))
    ...
    fun gradientRadiusInPixel(radius: Float) = apply {
        gradientRadius(radius); gradientRadiusType(
        DrawableProperties.RADIUS_TYPE_PIXELS
    )
    }

    fun gradientRadiusInFraction(radius: Float) = apply {
        gradientRadius(radius); gradientRadiusType(
        DrawableProperties.RADIUS_TYPE_FRACTION
    )
    }
